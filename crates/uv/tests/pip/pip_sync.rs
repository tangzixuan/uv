...
fn pep_751_empty_hashes() -> Result<()> {
    ...
    exit_code: 0 (success)
    ----- stderr -----
    warning: Empty hash tables in `pylock.toml` will be rejected in a future uv version. Run `uv export --format pylock.toml -o pylock.toml` to regenerate the file.
    Would download 1 package
    Would install 1 package
     + a==1.0.0
    ");
